 ...
 */
public class Recv {
	private final static String EXCHANGE_NAME = "direct_logs"; //设置一个交换器名称
	public static void main(String[] args) {
		...
		try {
			...
			channel.exchangeDeclare(EXCHANGE_NAME, "direct");//指定交换器名称和类型，和生产者保持一致
			String queueName = channel.queueDeclare().getQueue(); //获取信道的队列默认名称
			
			String[] logs = new  String[]{"debug","info"};//指定需要过滤的日志级别（也就是路由键，路由键匹配的消息会处理，否则会抛弃）
